...
const processWebhook = async({shop, payload})=>{
    try {
        if(shop && payload){
            ...
            if(subscriptionDataFound && payload.app_subscription.status !== "DECLINED"){
                if(payload.app_subscription.status !== "CANCELLED"){
                    await Subscription.findOneAndUpdate({shop}, {$set:{
...
export const action = async({request})=>{
    try {
        ...
        return new Response({status: 500, success: false, error:"Internal server error."})
    }
}