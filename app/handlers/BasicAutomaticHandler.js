...
import { DISCOUNT_AUTOMATIC_BASIC } from '../graphql/mutations/basic/automatic';

export default class BasicAutomaticHandler extends BaseDiscountHandler {
  constructor() {
    super();
    this.mutation = DISCOUNT_AUTOMATIC_BASIC;
    this.inputKey = "automaticBasicDiscount";
  ...
  buildInput(data) {
    // Build input according to DiscountAutomaticBasicInput schema
    const input = {
    ...
    // Optional fields based on DiscountAutomaticBasicInput
    if (data.end_date) {
  ...
  handleResponse(response) {
    // Handle response from discountAutomaticBasicCreate mutation
    ...
    if (!response.automaticDiscountNode) {
      throw new Error("No automatic discount node returned from API");
    }

    const discount = response.automaticDiscountNode.automaticDiscount;
    return {
